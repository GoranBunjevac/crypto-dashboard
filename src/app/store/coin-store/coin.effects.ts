...
import { selectFiatCurrency } from './coin.selectors';

@Injectable()
export class CoinEffects {
  constructor(private service: CoinService, private actions$: Actions, private store$: Store<CoinState>) { }

  @Effect()
  public loadCoins$ = this.actions$
    .pipe(ofType<CoinLoadAction>(CoinActionType.LOAD_REQUEST),
      switchMap(action =>
        this.service
        .getCoinData(action.fiatCurrency)
        .pipe(
          map((response: Coin[]) => new CoinLoadSuccessAction(response, action.fiatCurrency)),
